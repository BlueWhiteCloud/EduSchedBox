...
function startPackagedServer() {
  try {
    ...
    if (fs.existsSync(serverExe)) {
      packagedServerProcess = spawn(serverExe, [], { windowsHide: true, stdio: ['ignore', 'pipe', 'pipe'] })
      packagedServerProcess.stdout?.on('data', (d: Buffer) => console.log('[server]', d.toString()))
      packagedServerProcess.stderr?.on('data', (d: Buffer) => console.error('[server]', d.toString()))
      packagedServerProcess.on('exit', (code: number) => { console.log('Packaged server exited', code); packagedServerProcess = null })
...
function createWindow() {
  ...
  if (process.env.VITE_DEV_SERVER_URL) {
    ...
    const indexPath = app.isPackaged ? path.join(process.resourcesPath, 'dist', 'index.html') : path.join(__dirname, '../dist/index.html')
  ...
  ipcMain.handle('generate:openWindow', () => {
    ...
    if (process.env.VITE_DEV_SERVER_URL) {
      ...
      const genIndexPath = app.isPackaged ? path.join(process.resourcesPath, 'dist', 'index.html') : path.join(__dirname, '../dist/index.html')
  ...
  ipcMain.handle('generate:request', async (_e, payload) => {
    ...
    try {
      ...
      ipcMain.handle('generate:forceRefetch', async (_e2, p) => {
        try {
        ...
        try {
          ...
          if (res.ok) {
              ...
              try {
                ...
                const meta: any = { source: 'deepseek' }
                ...
                fs.appendFileSync(logFile, `${new Date().toISOString()}: attempt ${attempt+1} res.json failed: ${(e as Error).message}\n`)
          ...
          clearTimeout(timer)
          const took = Date.now() - attemptStart
          fs.appendFileSync(logFile, `${new Date().toISOString()}: attempt ${attempt+1} failed: ${(err as Error).message}, took ${took}ms\n`)
          console.warn('generate:request http call attempt failed', { name: (err as Error)?.name, message: (err as Error)?.message, attempt: attempt+1, timeoutMs, took })
          lastErr = err
      ...
      console.warn('generate:request http failed after retries, will fallback to python spawn/mock', (lastErr as Error)?.message)
    } catch (err) {
      fs.appendFileSync(logFile, `${new Date().toISOString()}: unexpected error before attempts: ${(err as Error).message}\n`)
      console.warn('generate:request unexpected error before http attempts, falling back to python spawn/mock:', (err as Error)?.message)
    }
    ...
    try {
      ...
      const res = await new Promise<any>((resolve, reject) => {
        ...
        proc.on('close', (code) => {
          ...
          try {
            ...
            reject(new Error('invalid json from python: ' + (e as Error).message))
          }
        })
      ...
      const returnedMeta: any = { source: 'python', provisional: true }
      ...
      // After falling back to python result, schedule background attempts to re-fetch DeepSeek result
          ...
          for (let i = 0; i < attempts; i++) {
            try {
              ...
              if (r.ok) {
                ...
                BrowserWindow.getAllWindows().forEach(w => w.webContents.send('generate:updated', { ...json, __meta: { source: 'deepseek', updated: true } }))
                return
              } else {
              ...
              console.log('generate:request background refetch attempt failed', i+1, (e as Error)?.message)
            }
            await new Promise(r => setTimeout(r, 10000))
          }
          console.log('generate:request background refetch exhausted attempts')
        } catch (e) {
          console.error('generate:request background refetch error', (e as Error)?.message)
        }
      })()
      return { ...res, __meta: returnedMeta }
    } catch (err) {
      fs.appendFileSync(logFile, `${new Date().toISOString()}: python spawn failed: ${(err as Error).message}\n`)
      console.warn('generate:request python call failed, falling back to mock', (err as Error)?.message)
    }
    // fallback mock
  ...
  ipcMain.handle('generate:basicRequest', async (_e, payload) => {
    ...
    try {
      ...
      try {
        ...
        console.warn('generate:basicRequest http call failed', { name: (err as Error)?.name, message: (err as Error)?.message })
      }
    } catch (err) {
      console.warn('generate:basicRequest unexpected error before http attempts, falling back to python spawn/mock:', (err as Error)?.message)
    }
  ...
  ipcMain.handle('window:closeMe', (event) => {
    try {
  ...
  ipcMain.handle('calendar:notifyUpdated', (event, payload) => {
    try {
...
app.whenReady().then(() => {
  ...
  ipcMain.handle('theme:get-themes', async () => {
    ...
    if (!fs.existsSync(colorsDir)) return {}
  ...
  ipcMain.handle('theme:save-theme', async (_e, name: string, themeData: any) => {
    const fs = require('fs')
    const path = require('path')
    let colorsDir = path.join(__dirname, '../src/assets/colors')
    if (!fs.existsSync(colorsDir)) {
    ...
    if (!fs.existsSync(colorsDir)) {
      try { fs.mkdirSync(colorsDir, { recursive: true }) } catch (e) {}
    }
  ...
  ipcMain.handle('theme:delete-theme', async (_e, name: string) => {
    const fs = require('fs')
    const path = require('path')
    let colorsDir = path.join(__dirname, '../src/assets/colors')
    if (!fs.existsSync(colorsDir)) {
